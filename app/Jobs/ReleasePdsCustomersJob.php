...
class ReleasePdsCustomersJob implements ShouldQueue, ShouldBeUnique
{
    ...
    {
        $dialerPayload = $action->releasePdsCustomers($this->pdsId, $this->companyId);

        if (!$dialerPayload) {
            return;
        }

        $dialer = Dialer::post('/campaign-dialer/releaseCustomerPDS', $dialerPayload, true);
